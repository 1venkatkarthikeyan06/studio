...
const AnalyzeAnswerInputSchema = z.object({
  ...
  role: z.string().describe('The job role the candidate is interviewing for (e.g., "Software Engineer").'),
});
...
const AnalyzeAnswerOutputSchema = z.object({
  ...
  feedback: z.object({
    clarity: z.string().describe('Feedback on the clarity and conciseness of the answer.'),
    relevance: z.string().describe('Feedback on how relevant the answer is to the question and the specified job role.'),
    speechPace: z.string().describe('Feedback on the candidate\'s pace of speech. Assume a standard speaking rate. This is only applicable for voice-based interviews.')
...
2.  Perform Named Entity Recognition (NER) on the given answer transcript and anonymize it.
    ...
    - Create a mapping table (entityMap) that links the original entity to its anonymized ID and includes the entity type.
